...
%\subtitle{\Large COMP6741: Parameterized and Exact Computation}
...
\begin{frame}{Coping with NP-hardness}
 \begin{itemize}
  ...
  \begin{itemize}
%    \item There is an algorithm, which, given an instance $(G,k)$ for \textsc{Vertex Cover}, finds a vertex cover of size at most $2k$ or correctly determines that $G$ has no vertex cover of size $k$.
...
\begin{center}
 \begin{tikzpicture}
  ...
  \draw %(5) to node[swap,auto] {$3$} (6)
             (5) to node[swap,auto] {$7$} (7)
        %     (5) to node[auto] {$4$} (8)
             (5) to node[auto] {$13$} (9)
...
%  \item interesting combinatorics
  \bigskip
...
%nb. of operations& $2^{36}$& $2^{42}$& $2^{48}$& $2^{54}$& $2^{60}$\tabularnewline
%\midrule
%$n^{5}$& 147& 337& 776& 1782& 4096\tabularnewline
%%\hline
%$n^{10}$& 12& 18& 27& 42& 64\tabularnewline
%%\hline
%%$n^{20}$& 3& 4& 5& 6\tabularnewline
%%\hline
%$1.05^{n}$& 511& 596& 681& 767& 852\tabularnewline
%%\hline
%$1.1^{n}$& 261& 305& 349& 392& 436\tabularnewline
%%\hline
%$1.5^{n}$& 61& 71& 82& 92& 102\tabularnewline
%%\hline
%$2^{n}$& 36& 42& 48& 54& 60\tabularnewline
%%\hline
%$5^{n}$& 15& 18& 20& 23& 25\tabularnewline
%%\hline
%$n!$& 13& 15& 16& 18& 19\tabularnewline
%\bottomrule
...
\begin{frame}
 ...
 \begin{center}
  \includegraphics[height=2cm]{pc.jpg}
 \end{center}
...
%\subsection{Examples of parameters}
...
%\subsection{Main Complexity Classes} % ($P \subseteq FPT \subseteq W[1] \subseteq W[2]$)
...
%	\frametitle{Exercise}
%	\pause
%	Show that VC can be solved in polynomial time for graphs of maxmium degree at most $2$.
%	\slides{\vspace{7.5cm}}
%\end{frame}
%
%\teacher{
...
%		Show that VC can be solved in polynomial time for graphs of maximum degree at most $2$.
%		\vspace{0.5cm}
%		\begin{itemize}
%			\item A graph of maximum degree at most 2 is a disjoint union of paths and cycles
%			\item Denote by $vc_{opt}(G)$ the size of a smallest vertex cover of $G$
%		\end{itemize}
%		\begin{lemma}
%			For a path $P_k$ on $k\ge 1$ vertices, $vc_{opt}(P_k) = \lceil (k-1)/2\rceil$.
%		\end{lemma}
%		\begin{proof}[Proof sketch]
%			By induction on $k$.\\
%			Base cases: check for $k=1$ and $k=2$.\\
%			Induction: Let $k\ge 3$ and assume the lemma is true for $P_{k'}$ for all $k'$, $1\le k'<k$. One can prove that $vc_{opt}(P_k) = 1+vc_{opt}(P_{k-2}) = 1+\lceil (k-3)/2 \rceil = \lceil (k-1)/2 \rceil$.
%		\end{proof}
%		\begin{itemize}
%			\item For a cycle $C_k$ on $k\ge 3$ vertices, $vc_{opt}(C_k) = 1+vc_{opt}(P_{k-1}) = \lceil k/2 \rceil$.
%		\end{itemize}
%	\end{frame}
%}
...
%
%\begin{frame}
%	\slides{\frametitle{Administrative matters}}
%	
%	\begin{itemize}
%		\item Enrolments
%		\item Website
%		\item Lectures; exercises, questions, consultations
%		\item Breaks
%		\item Survey
%		\item Course and assignment schedule
%		\item Mid-session quiz
